...
def calculate_accuracy(trainloader, testloader, is_gpu):
    ...
    epoch = 2
    checkpoint = torch.load(f"../checkpoint/{epoch}_checkpoint.pth.tar")
    ...
    np.save(f"../{epoch}_embedded_features_train", embedded_features_train)

    # """
    # embedded_features_train = np.load(f"../{epoch}_embedded_features_train.npy")
    print(embedded_features_train.shape)
    ...
    with torch.no_grad():
        for test_id, test_data in enumerate(testloader):
            ...
            # print(embedded_test_numpy.shape)
            ...
            min_index = embedding_norm.argsort()[:topk]
            ...
            # FOR TEST
            # print("test class: ", test_image_class, test_image_name)
            # plt.subplot(10, 11, test_id * 11 + 1)
            # test_image_path = f"/home/colin/Github/Computer-Vision/data/tiny-imagenet-200/val/images/{test_image_name}"
            # test_img = Image.open(test_image_path.rstrip("\n")).convert('RGB')
            # plt.imshow(test_img)
            # plt.show()
            ...
            for i, idx in enumerate(min_index):
                ...
                # get result image class, be carefull with the classname!!!
                top_result_image_name = training_images[idx]
                top_result_image_name_class = top_result_image_name.split(
                    "/")[5]

                # FOR TEST
                # print(top_result_image_name_class)
                # plt.subplot(10, 11, test_id * 10 + i + 2)
                # res_image = Image.open(top_result_image_name.rstrip("\n")).convert('RGB')
...
def get_classes(filename="../../data/tiny-imagenet-200/val/val_annotations.txt"):
    """
...
def main():
    ...
    parser.add_argument('--dataroot', type=str, default="/home/colin/Github/Computer-Vision/data/tiny-imagenet-200",
